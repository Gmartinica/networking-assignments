...
class Window:
    def __init__(self, pkg_list):
        self.seq_list = [p.seq_num for p in pkg_list]
        self.num_frames = len(pkg_list)
        self.window_size = 1 if len(pkg_list) <= 2 else 2
        self.seq_start = 0
        self.window = {num: False for num in self.seq_list}  # Create dict with false values for each packet at start
